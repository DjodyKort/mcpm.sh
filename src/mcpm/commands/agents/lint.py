...
from mcpm.skills.agents.lint import lint_agents
...
def lint_agents_cmd(path):
    """Validate agents and check for best-practice issues.
    ...
    if not found_agents:
        console.print("[yellow]No agents found to lint.[/]")
        return

    result = lint_agents(found_agents)

    if not result.messages:
        console.print(f"[green]All {len(found_agents)} agent(s) passed lint checks.[/]")
        return
    ...
    for msg in result.messages:
        if msg.level == "error":
            ...
            icon = "[dim]I[/]"
        console.print(f"  {icon} [cyan]{msg.agent_name}[/]: {msg.message}")
